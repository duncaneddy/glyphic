    ...
    <div class="space-y-2 text-sm">
      <template v-if="editor.config.content.type === 'url'">
        <input v-model="editor.config.content.url" placeholder="https://example.com"
          class="w-full rounded border border-gray-300 px-2 py-1.5 dark:border-[#4a4a4a] dark:bg-[#333333] dark:text-neutral-100" />
      </template>
      ...
      <template v-else-if="editor.config.content.type === 'wifi'">
        ...
        <div class="flex gap-2">
          <select v-model="editor.config.content.encryption"
            class="rounded border border-gray-300 px-2 py-1.5 dark:border-[#4a4a4a] dark:bg-[#333333] dark:text-neutral-100">
          ...
          <input v-if="editor.config.content.encryption !== 'nopass'"
            ...
            class="flex-1 rounded border border-gray-300 px-2 py-1.5 dark:border-[#4a4a4a] dark:bg-[#333333] dark:text-neutral-100" />
        </div>
        <label class="flex items-center gap-2 text-xs text-gray-600 dark:text-neutral-400">
      ...
      <template v-else-if="editor.config.content.type === 'location'">
        <div class="flex gap-2">
          <input v-model.number="editor.config.content.latitude" type="number" step="any"
            placeholder="Latitude" class="flex-1 rounded border border-gray-300 px-2 py-1.5 dark:border-[#4a4a4a] dark:bg-[#333333] dark:text-neutral-100" />
          <input v-model.number="editor.config.content.longitude" type="number" step="any"
            placeholder="Longitude" class="flex-1 rounded border border-gray-300 px-2 py-1.5 dark:border-[#4a4a4a] dark:bg-[#333333] dark:text-neutral-100" />
        </div>
      </template>
    </div>
  </SectionCard>
